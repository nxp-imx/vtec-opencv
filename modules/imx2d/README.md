...
Purpose of this module is to accelerate execution of OpenCV functions by using i.MX 2D hardware acceleration hardware.

Current version only supports hardware acceleration of scaling primitive `cv::resize()`.
...
Current implementation run on platforms:
- i.MX 8M Plus (GPU2D)
- i.MX 93 (PXP)
- i.MX 8QuadMax (DPU)
- i.MX 8QuadXPlus (DPU)
...
1. An Hardware Adaptation Layer library implementing the custom version of the overridden functions.

2. imx2d extra module, providing implementation for:
    * Dedicated interfaces exposed to the user applications
    * Mat buffer allocator based on graphic buffers
    * Some utility classes shared with HAL
...
OpenCV common container for images is the `Mat` (matrix) container implemented by the [`cv::Mat`](https://docs.opencv.org/4.x/d3/d63/classcv_1_1Mat.html) class. It consists in a reference to a buffer and a large set of attributes defining the properties of the embedded data such as its data type, dimensions, number of channels etc.


### System memory buffers

Using standard implementation, `Mat` buffers are allocated from:
- System memory - that is `malloc()`- when created from C++ API
- Numpy buffer, backed by system memory, when created from Python bindings
In both cases, that is user space virtual memory that is mapped to CPU though MMU, hence it is not continuously mapped in physical memory.
...
Hardware accelerators on the other hand require usage of continuous memory that is accessed by devices through DMA physical bus address. Such buffers are later references Graphic memory buffers, short-names as 'Gi' for 'graphic input buffer', and 'Go' for 'graphic output buffer'.
Several options are possible to process `Mat` buffers from hardware accelerators, listed in following sections.
Option used depends on:
- Custom Mat graphic allocator enablement
- Underlying hardware accelerator support for 3 channels operations


### Mat buffers backed by system memory

In that case intermediate buffer copies are needed to have the image stored in `Mat` container processed by the hardware accelerator.

![Mat buffer cycle - system memory](media/opencv2d_mat_sys_memory_cycle.png)

Sequence of actions is as follow:
1. User applications submits `Mat` objects with respective input (Si) and output (So) buffers backed in system memory
...
Enabling the custom Mat allocator, `Mat` buffers instantiated by the application will be allocated from graphic memory pool, instead of system memory.
...
Sequence of actions is as follow:
1. User applications submits `Mat` objects with respective input (Gi) and output (Go) buffers backed in graphic memory
...
GPU2D and PXP accelerators support 4 channels operations like BGRA but not 3 channels like BGR that is default OpenCV color format.

For platforms where only 4 channels operations are supported, operation on `Mat` containers based on 3 channels image is done, by adding intermediate steps:
...
Such buffer color format conversions are done relying on OpenCV conversions primitives that are:
1. Neon accelerated
2. multi-threaded

However such emulated support is beneficial at the condition that load associated to format conversion remains small compared to the processing time of the 2D operation when executed on CPU.
In that case conversion overhead on the CPU is compensated by the gain of delegating the execution of the 2D operation to the hardware accelerator.

In such case where color format conversion is required before/after accelerated 2D operation, it does not matter if input and output `Mat` containers are based on system or graphic memory buffers. Color format converted images will consistently be stored in graphics buffers so that they are accessible to the accelerator device.
...
Interfaces exported by `imx2d` module are listed in its [public header file](./include/opencv2/imx2d.hpp), that documents details of the interface.
All definitions from public API belong to the `cv::imx2d::` namespace. Associated Python bindings, when available, belong to cv2.imx2d sub-package.
...
By default module is disabled so will not provide acceleration.
...
Interface is provided to enable activation of this custom `MatAllocator`, and to query its status.
...
1. Minimum `Mat` buffer size to be allocated as a graphic buffer. Can be used by applications that use many smaller `Mat` containers holding non-graphic data
2. Graphic buffer CPU cache configuration. Cached buffers have the downside of requiring cache coherency operations (flush, invalidate) but have much faster CPU access

Interface is provided to configure those parameters in the graphic memory `MatAllocator`.
...
Graphic buffer are allocated by the module for:
...
Allocation of a new graphic buffer takes some time. Moreover some graphic buffers may have to be frequently allocated and deallocated, for instance intermediate buffers in use case [Mat buffers in system memory](#mat-buffers-backed-by-system-memory).
...
Primitives can be accelerated providing that a certain number of condition are met on the `Mat` container data, that have to be compatible with acceleration hardware capabilities.

When acceleration is not possible, implementation falls back onto alternative OpenCV implementation, that may be provided by [Carotene HAL](https://github.com/opencv/opencv/tree/4.x/3rdparty/carotene) with NEON optimization or the default OpenCV CPU implementation.
...
A cpp sample application exercising the module on a [`VideoCapture`](https://docs.opencv.org/4.x/d8/dfe/classcv_1_1VideoCapture.html) input stream is provided [here](./samples/camera_resize.cpp). It gives the option to run the same video operation, with i.MX2D acceleration and/or graphic `Mat` allocator enabled.

This application uses `highgui` OpenCV module that relies on `Qt` framework. Therefore, in order to have a functional GUI, application must be built with an SDK including the `Qt` packages.

Installation path of the application is typically `/usr/bin/imx2d` when built and installed with [build.sh](../build.sh). For convenience shell `PATH` variable may be adjusted accordingly:

```bash
$ PATH="/usr/bin/imx2d:${PATH}"
```
Input and output video and execution options can be configured via command line.
Options can be listed with the help command argument.
```bash
$ example_imx2d_video_test --help
```
Usage example to capture stream stream from `/dev/video3` device using V4L2 backend:
```bash
$ example_imx2d_video_test --cv4l2 --cid=3
```
Usage example to capture stream from [`gst-launch`](https://gstreamer.freedesktop.org/documentation/tools/gst-launch.html?gi-language=c) pipeline using `GStreamer` backend:
```bash
$ example_imx2d_video_test --cgst --cstr="videotestsrc ! videoconvert ! appsink"
```
Usage example to capture video stream from input camera, having i.MX 2D acceleration enabled but disabling the graphic `Mat` allocator:
```bash
$ example_imx2d_video_test --cid=0 --imx2d=1 --alloc=0
```
...
Performance tests verify for multiple use cases (channels, resolution, memory backing buffers...
) the output of a 2D operation and also compare the execution time with different backend: 2D accelerated, CPU, OpenCL (when available).
